...
class DirectBookingBottomSheet : BottomSheetDialogFragment() {
    ...
    private fun bindActionButtons(v: View) {
        ...
        when (booking.bookingStatus) {
            ...
            "Approved" -> {
                // Pickup button only appears on or after the start date (mirrors web logic)
    ...
    private fun undoPickup() {
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ApiClient.adminApi.undoPickup(ApiClient.bearerToken(), booking.id)
                toast("Pickup undone")
                onReload?.invoke()
                dismiss()
            } catch (_: Exception) { toast(getString(R.string.error_network)) }
    ...
    // ── Sub-dialogs (delegate to parent so CalendarPickerDialog uses correct fm) ─
